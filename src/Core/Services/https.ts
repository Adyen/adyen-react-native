import AdyenCheckoutError from '../AdyenCheckoutError';
...
function isAdyenErrorResponse(data: any): data is AdyenErrorResponse {
  return (
    data && data.errorCode && data.errorType && data.message && data.status
  );
}
...
function getURL(environment: Environment): string {
  return `https://checkoutshopper-${environment}.adyen.com/checkoutshopper/`;
}
