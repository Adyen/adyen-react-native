...
export class ContextModuleWrapper implements AdyenContextModule {
  ...
  provideBeforeSubmitResult(result: BeforeSubmitResult): void {
    this.nativeModule.provideBeforeSubmitResult(result);
  }
   ...
   * @param callback - Called after component validation and before the SDK submits payment data.
   * @returns EventSubscription that can be used to remove the listener.
   */
  assignBeforeSubmitHandler(
