...
| `showRemovePaymentMethodButton`                                            | Enables UI that allows the shopper to remove stored payment methods. Defaults to **false**. For `\sessions` flow this option works out of the box. For `\payment` (aka "Advanced") flow one must also use `onDisableStoredPaymentMethod?` callback.                                        | No       |
| `onDisableStoredPaymentMethod(storedPaymentMethod, resolve, reject) => {}` | Callback from Drop-In to call [/disable](https://docs.adyen.com/api-explorer/Recurring/68/post/disable) API endpoint. This callback only used when`showRemovePaymentMethodButton` is **true** and shopper decided to remove stored payment method during `\payment` (aka "Advanced") flow. | No       |
