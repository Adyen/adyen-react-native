...
The v6 API provides specialized result handlers for each callback:

| Handler | Available methods | Used in |
| --- | --- | --- |
| `PaymentSubmitResultHandler` | `action()`, `completion()`, `retry()` | `onSubmit` (advanced flow) |
| `PaymentAdditionalResultHandler` | `completion()` | `onAdditionalDetails` (advanced flow) |
| `PaymentResultHandler` | `completion()` | `onComplete`, `onError` (both flows) |
